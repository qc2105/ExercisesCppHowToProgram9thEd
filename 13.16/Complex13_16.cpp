...
name13_16::Complex::Complex(int Real, int Imaginary)
	:real(Real),
	imaginary(Imaginary),
	inGoodState(true)
{
...
void name13_16::Complex::removeSpaces(std::string & s) const
{
...
bool name13_16::Complex::fectchNextInteger(const std::string & s, size_t & index, int & integer) const
{
	if (s.empty())
	{
		return false;
...
namespace name13_16 {
	std::ostream& operator << (std::ostream& output, const name13_16::Complex& c)
	{
		output.clear();
		if (c.inGoodState)
		{
	...
	std::istream& operator >> (std::istream& input, name13_16::Complex& c)
	{
		input.clear();
