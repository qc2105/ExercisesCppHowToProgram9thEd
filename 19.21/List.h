...
namespace name19_21 {
	...
	public:
		...
		{
			reversePrintHelper(&headPtr);
			std::cout << std::endl;
		}

		Node<T>* searchList(const T& data) const
		{
			Node<T>* resultPtr = nullptr;

			searchListHelper(&headPtr, data, resultPtr);
	...
	private:
		...
		void reversePrintHelper(Node<T>*const * headPtr) const
		{
			if ((*headPtr)->nextPtr != nullptr)
			{
				reversePrintHelper(&((*headPtr)->nextPtr));
			}

			std::cout << (*headPtr)->data << ' ';
		}

		void searchListHelper(Node<T>*const * headPtr, const T& data, Node<T>*& ptr) const
		{
			if ((*headPtr)->nextPtr != nullptr)
			{
				searchListHelper(&((*headPtr)->nextPtr), data, ptr);
			}
