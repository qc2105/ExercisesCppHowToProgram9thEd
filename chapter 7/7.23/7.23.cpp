// 7.23.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
int main()
{
	...
	std::vector<int> toursVec;
	KnightTour testRun;

	do {
		int initRow = rand() % 8;
		int initColumn = rand() % 8;
		testRun.initialize(initRow, initColumn);
		while (testRun.mCount <= 64)
		{
			testRun.move(testRun.nextMoveType());
			if (testRun.isTheEnd())
			{
				break;
			}
		...
		toursVec.push_back(testRun.mCount);
	} while (testRun.mCount < 63);
	
	std::cout << "Best tour: " << *std::max_element(toursVec.begin(), toursVec.end());
